...
class _ControlConnectedDevicePageState
  ...
  late final BluetoothState bluetoothState;
...
//start the animation
  ...
  initializeBluetoothListener() async {
    setState(() {
      isConnecting = false;
    });
    if (widget.bluetoothState == BluetoothDeviceState.connected) {
      _services = await widget.bluetoothConnection.discoverServices();

      for (BluetoothService service in _services) {
        for (BluetoothCharacteristic c in service.characteristics) {
           if (c.uuid == new Guid("9e8fafe1-8966-4276-a3a3-d0b00269541e")) {
            //final mtu = await widget.bluetoothConnection.mtu.first;
            await widget.bluetoothConnection.requestMtu(251);
            isConnecting=await c.setNotifyValue(true);
            await Future.delayed(Duration(seconds: 10));
            c.value.listen((data) {
              if (data. isNotEmpty) {
                //Data entry point
                print("############################");
                print(widget.bluetoothState);
                ...
                if (jsonData["p"] != null) {
                  ...
                  if (cardKey.currentState!.isFront) {
                    ...
                    setState(() {
                      ...
                      massageConfiguration =
                          jsonEncode(processListWithoutEmptyPortion);
                      usedTime = totalTime;
                    });
                  ...
                  int totalProcessTime = processListWithoutEmptyPortion.fold(
                          0, (p, c) => int.parse(p.toString()) + (c["d"] * 60))
                      as int;
                  ...
                  totalProcessTime = (totalProcessTime -
                          (int.parse(
                                  jsonData["p"][currentStep]["d"].toString()) *
                              60)) +
                      currentProcessTime;
    ...
    if (text.length > 0) {
      try {
        ...
        if (widget.bluetoothState == BluetoothDeviceState.connected) {
          ...
          for (BluetoothService service in _services) {
            for (BluetoothCharacteristic c in service.characteristics) {
              //if (c.uuid == new Guid("9393c756-78ea-4629-a53e-52fb10f9a63f")) {
              if (c.uuid == new Guid("9393c756-78ea-4629-a53e-52fb10f9a63f")) {
               // List<int> bytes = utf8.encode(textToSend);
                c.write(utf8.encode(textToSend));
                
                // processList.asMap().forEach((index, element) {
                //   String process = '';
                //   if (element['s'] <= 50) {
                //     process = 'A';
                //   } else if (element['s'] > 50 && element['s'] <= 100) {
                //     process = 'B';
                //   } else if (element['s'] > 100) {
                //     process = 'C';
                //   } else if (element['s'] == 0) {
                //     process = 'D';
                //   }
                //   totaltime += int.parse(element['d'].toString());

                //   var characteristics = service.characteristics;
                //   for (BluetoothCharacteristic c in characteristics) {
                //     c.write(utf8.encode(process));
                //   }
                // });
              }
            }
...
//ignore: unnecessary_statements
        ...
        // widget.bluetoothConnection..add(bytes);
        // await widget.bluetoothConnection!.canSendWriteWithoutResponse;
  ...
  void dispose() {
    ...
    widget.bluetoothConnection.disconnect();
    // connection = null;
