...
void ProcessData(String data, bool isBluetooth) {
  ...
  char* motorSpeed = doc["speed"];
  int duration = doc["duration"];

  switch (toupper(motorSpeed[0])) {
    case 'L':
      MassagerRun(80, duration, isBluetooth);       //pwm Value and massaging mode
      Serial.println ("exit main switchcase");
      break;
    case 'M':
      MassagerRun(120, duration, isBluetooth);
      Serial.println ("exit main switchcase");
      break;

    case 'H':
      MassagerRun(180, duration, isBluetooth);
      Serial.println ("exit main switchcase");
      break;
    default:
